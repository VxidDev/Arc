...
Chunk *initChunk(void) {
  ...
  chunk->count = 0; chunk->capacity = CHUNK_INIT_CAP;
...
void chunkWrite(Chunk *chunk, uint8_t byte) {
  ...
  if (chunk->count >= chunk->capacity) {
    ...
    uint8_t *grown = arenaRealloc(objectArena, chunk->code,
                                  oldCap * sizeof(uint8_t),
                                  newCap * sizeof(uint8_t));
    if (!grown) return;
    chunk->code = grown; chunk->capacity = newCap;
  }
...
int chunkAddConst(Chunk *chunk, Object *obj) {
  ...
  if (chunk->constCount >= chunk->constCapacity) {
    ...
    size_t newCap = oldCap * 2;
    Object **grown = arenaRealloc(objectArena, chunk->constants,
                                  oldCap * sizeof(Object *),
                                  newCap * sizeof(Object *));
    if (!grown) return -1;
...
void freeChunk(Chunk *chunk) {
  if (!chunk) return;
  for (size_t i = 0; i < chunk->constCount; i++) freeObject(chunk->constants[i]);
...
static uint8_t addConst(Compiler *c, Object *obj) {
    ...
    if (idx < 0 || idx > 255) {
        if (c->err && !*c->err)
            *c->err = initRuntimeError((Position){0,0,0}, (Position){0,0,0},
                c->filename, "Too many constants in chunk (max 256).", c->sourcetext);
        return 0;
    }
    return (uint8_t)idx;
}
...
static void compileNumber(ASTNode *node, Compiler *c) {
  ...
  Object *obj = num->token.type == TOK_FLOAT
                  ? (Object *)initFloat(num->token.val.f)
                  : (Object *)initInt(num->token.val.i);
...
static void compileIf(ASTNode *node, Compiler *c) {
  ...
  for (int i = 0; i < endJumpCount; i++) patchJump(c, endJumps[i]);
}
...
static void compileTryCatch(ASTNode *node, Compiler *c) {
  ...
  Object *name = (Object *)initString(tn->errIdentifier.val.s,
                                      strlen(tn->errIdentifier.val.s));
  emitBytes(c, OP_STORE_VAR, addConst(c, name));
  emitByte(c, OP_POP);
...
static void compileImport(ASTNode *node, Compiler *c) {
  ...
  Object *path = (Object *)initString(in->filePath.val.s,
                                      strlen(in->filePath.val.s));
...
static void compileList(ASTNode *node, Compiler *c) {
  ...
  for (uint64_t i = 0; i < ln->size; i++) compileNode(ln->objects[i], c);
...
static void compileIndexAssign(ASTNode *node, Compiler *c) {
  ...
  Object *name = (Object *)initString(ia->targetIdent.val.s,
                                      strlen(ia->targetIdent.val.s));
...
static void compileProgram(ASTNode *node, Compiler *c) {
  ...
  for (size_t i = 0; i < prog->count; i++) {
    ...
    if (i < prog->count - 1) emitByte(c, OP_POP);
  }
}
...
Chunk *compileAST(ASTNode *ast, Error **err, char *filename, char *sourcetext) {
  ...
  if (ast->type == NODE_PROGRAM) compileProgram(ast, &c);
  else                           compileNode(ast, &c);
  emitByte(&c, OP_HALT);

  if (*err) { freeChunk(c.chunk); return NULL; }
  return c.chunk;
}

static void printConstant(Chunk *chunk, uint8_t idx) {
  if (idx >= chunk->constCount) { printf(" <invalid const %u>", idx); return; }
